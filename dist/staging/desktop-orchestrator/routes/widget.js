...
<body data-hana-theme="${escapeAttr(theme)}">
  <main class="panel">
    ...
    <section class="card danger" id="safetyCard">
      ...
      <div class="value" id="safetyValue">Real input blocked</div>
      <p id="safetyText">真实点击、鼠标移动、键盘输入仍需 dryRun=false、配置授权和确认短语。</p>
    </section>

    <section class="card composer">
      <div class="row between">
        <label for="bundleInput">Approval Bundle JSON</label>
        <span class="hint" id="parseHint">loading recent</span>
      </div>
      ...
      <div class="actions">
        <button type="button" id="parseButton">Parse preview</button>
        <button type="button" id="clearButton" class="secondary">Clear</button>
        <button type="button" class="disabled" disabled>Confirm disabled</button>
      </div>
    </section>
    ...
    <section class="card cockpit-summary-card" id="cockpitSummaryCard">
      <div class="row between">
        <div>
          <div class="label">Cockpit Summary</div>
          <div class="cockpit-headline" id="cockpitHeadline">等待协议健康检查。</div>
        </div>
        <button type="button" id="refreshCockpitSummaryButton" class="small secondary">Refresh summary</button>
      </div>
    ...
    <section class="card checklist-card">
      <div class="row between">
        <div>
          <div class="label">Approval Checklist</div>
          <p id="checklistMeta">等待审批包和预览证据。</p>
        </div>
        <div class="actions compact-actions">
          <button type="button" id="generateTokenButton" class="small secondary" disabled>Generate token</button>
          <button type="button" id="runPreflightButton" class="small secondary">Run preflight</button>
          <button type="button" id="buildFinalEnvelopeButton" class="small secondary">Final envelope</button>
          <button type="button" id="runSelfCheckButton" class="small secondary">Self-check</button>
          <button type="button" id="runProtocolMatrixButton" class="small secondary">Protocol matrix</button>
          <button type="button" id="runFixtureSandboxButton" class="small secondary">Fixture sandbox</button>
        </div>
      </div>
    ...
    <section class="card timeline-card">
      <div class="row between">
        <div>
          <div class="label">Audit Timeline</div>
          <p>审批证据链，本地只读展示。</p>
        </div>
        <div class="actions compact-actions">
          <button type="button" id="exportEvidenceButton" class="small secondary">Export evidence</button>
          <button type="button" id="refreshTimelineButton" class="small secondary">Refresh</button>
        </div>
      </div>
    ...
    <section class="card overlay-card">
      <div class="row between">
        <div>
          <div class="label">Cursor Overlay</div>
          <p id="overlayMeta">等待 cursorOverlay。</p>
        </div>
        <button type="button" id="replayOverlayButton" class="small secondary">Replay</button>
      </div>
    ...
    <section class="card raw-card">
      <div class="row between">
        <div class="label">Normalized Bundle</div>
        <span class="hint">local render only</span>
      </div>
...
(() => {
  ...
  function renderBundle(bundle) {
    ...
    els.safetyValue.textContent = blocked ? 'Real input blocked' : 'Real input gate open';
    els.safetyText.textContent = blocked ? '当前 bundle 仍处于预览模式。确认按钮保持禁用。' : 'bundle 显示真实动作门已开，但此 skeleton 仍不会执行动作。';
  ...
  function parseAndRender() {
    ...
    try {
      ...
      setHint(error.message || 'parse failed', 'error');
  ...
  async function loadRecent() {
    try {
      ...
      if (recent?.bundle) {
        ...
        setHint('loaded recent · ' + (recent.record?.source || 'store'), 'ok');
      } else {
        setHint(recent?.reason === 'no-live-approval-bundle' ? 'no live recent bundle · generate a fresh ui-tree/click-element bundle' : 'no recent bundle', 'warn');
      }
    } catch (error) {
  ...
  async function runExecutionPreflightFromWidget() {
    ...
    els.preflightOutput.textContent = 'running preflight...';
    if (!approvalState.approvalTokenRecordId) {
      ...
      els.preflightOutput.textContent = JSON.stringify({ ok: true, passed: false, status: 'waiting', statusLabel: 'waiting', headline: 'Generate a fresh approval token before running preflight.', reason: 'approval-token-not-bound-to-widget' }, null, 2);
  ...
  async function runFixtureSandboxFromWidget() {
    ...
    els.fixtureSandboxOutput.textContent = 'running fixture sandbox...';
    try {
  ...
  async function runProtocolMatrixFromWidget() {
    ...
    els.protocolMatrixOutput.textContent = 'running protocol matrix...';
    try {
  ...
  async function runSelfCheckFromWidget() {
    ...
    els.selfCheckOutput.textContent = 'running self-check...';
    try {
  ...
  async function buildFinalEnvelopeFromWidget() {
    ...
    els.finalEnvelopeOutput.textContent = 'building final dry-run envelope...';
    if (!approvalState.approvalTokenRecordId) {
      els.finalEnvelopeOutput.classList.remove('ready');
      els.finalEnvelopeOutput.classList.add('blocked');
      els.finalEnvelopeOutput.textContent = JSON.stringify({ ok: true, readyForHumanFinalReview: false, blocked: true, blockedReasons: ['approval-token-not-bound-to-widget'], executable: false, executionMode: 'dry-run-only', headline: 'Generate a fresh approval token before building the final envelope.' }, null, 2);
  ...
  els.generateTokenButton.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!snapshotStatus?.ok) {
        approvalState.approvalTokenRecordId = null;
        els.approvalTokenOutput.textContent = JSON.stringify({ token, snapshotStatus, saved: null, status: 'waiting', headline: 'Approval bundle snapshot is missing or stale. Generate a fresh approval bundle before creating a token.' }, null, 2);
        return;
      }
