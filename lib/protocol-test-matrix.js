...
import { isRealActionBlocked, requireRealInputApproval, REAL_INPUT_CONFIRMATION } from "./safety.js";
...
export function runProtocolTestMatrix() {
  ...
  const confirmationBypassAttempt = requireRealInputApproval(
    { dryRun: false },
    { allowRealInput: true, skipConfirmationPhrase: true },
    { actionType: "manage-window", action: { type: "close" } },
  );
  cases.push(caseResult("confirmation-cannot-be-skipped-by-config", Boolean(
    confirmationBypassAttempt?.allowed === false
      && confirmationBypassAttempt?.requiresConfirmation === true
      && String(confirmationBypassAttempt?.reason || "").startsWith(`需要确认短语 ${REAL_INPUT_CONFIRMATION}`),
  ), {
    result: confirmationBypassAttempt,
  }));

  cases.push(caseResult("real-action-blocked-when-approval-denied", Boolean(
    isRealActionBlocked({ approvalAllowed: false, actionAllowed: false }) === true
      && isRealActionBlocked({ approvalAllowed: true, actionAllowed: false }) === true
      && isRealActionBlocked({ approvalAllowed: true, actionAllowed: true }) === false,
  )));
