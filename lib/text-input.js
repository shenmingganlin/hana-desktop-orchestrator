...
export function verifyFocusedElementIdentity({ focusedElement, handle, targetKey } = {}) {
  ...
  if (focusedHandle && focusedHandle !== "0" && focusedHandle !== expectedHandle) {
    return {
      ok: false,
      reason: "focused-element-window-mismatch",
      expectedHandle,
      actualHandle: focusedHandle,
  ...
  const matchedBy = automationId === expectedKey ? "automationId" : name === expectedKey ? "name" : null;
  if (!matchedBy) {
    return {
      ok: false,
  ...
  return {
    ok: true,
    reason: "focused-element-identity-match",
    matchedBy,
    expectedHandle,
    expectedKey,
    focusedElement: focused,
