...
function targetsMatch(left = {}, right = {}) {
  return Boolean(
    left.leaseId && left.snapshotId && left.elementId
...
export function findRecentLiveApprovalBundleRecord({ target = {} } = {}) {
  const store = readApprovalStore();
  return store.records.find((record) => {
    const candidateTarget = getTarget(record);
    return targetsMatch(candidateTarget, target) && isRecordSnapshotLive(record);
  }) || null;
}

export function validateApprovalBundle(bundle) {
  if (!bundle || bundle.type !== "desktop-orchestrator-approval-bundle") {
    return { ok: false, reason: "invalid-approval-bundle" };
  }
  if (bundle.version !== APPROVAL_BUNDLE_VERSION) {
    return { ok: false, reason: "unsupported-approval-bundle-version", expectedVersion: APPROVAL_BUNDLE_VERSION, actualVersion: bundle.version ?? null };
  }
  if (typeof bundle.bundleHash !== "string" || bundle.bundleHash.length === 0) {
    return { ok: false, reason: "approval-bundle-hash-required" };
  }
  const actualHash = hashApprovalBundle(bundle);
  if (bundle.bundleHash !== actualHash) {
    return { ok: false, reason: "approval-bundle-hash-mismatch", expectedHash: actualHash, actualHash: bundle.bundleHash };
  }
  return { ok: true, version: bundle.version, bundleHash: bundle.bundleHash };
}

export function saveApprovalBundle(bundle, { source = "unknown" } = {}) {
  const validation = validateApprovalBundle(bundle);
  if (!validation.ok) {
    return { ...validation, storePath: STORE_PATH };
  }
