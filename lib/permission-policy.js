...
function actionConfirmationAllows({ actionPolicy, config, mode, risk }) {
  // full-access is the single authorization layer: once real input is enabled,
  // action-level confirmation overrides and hard-confirmation floors do not add
  // another prompt. Identity, scope, foreground, and target guards remain in
  // their respective execution paths.
  if (normalizePermissionMode(mode) === PERMISSION_MODES.FULL_ACCESS) return true;
  if (actionPolicy.hardConfirmation === true) return false;
  if (actionPolicy.key === "window.close") return false;
  if (normalizeRisk(risk) === ACTION_RISKS.DESTRUCTIVE) return false;
...
export function decidePermission({ input = {}, config = {}, actionType = "unknown", action = null, target = null, capability = null, risk = null } = {}) {
  ...
  const confirmationAllowsAction = mode === PERMISSION_MODES.FULL_ACCESS
    ? true
    : actionPolicy.key === "unknown"
  ...
  const actionPolicyResult = {
    ...
    level: mode === PERMISSION_MODES.FULL_ACCESS
      ? CONFIRMATION_LEVELS.AUTO
      : (actionConfirmation.explicit ? actionConfirmation.level : actionPolicy.defaultLevel),
...
export function buildPermissionPolicySummary({ mode, actionType, risk, confirmationPolicy } = {}) {
  ...
  return {
    ...
    destructiveAlwaysConfirms: normalizedMode !== PERMISSION_MODES.FULL_ACCESS,
  };
}
