...
import { buildApprovalBundle, hashApprovalBundle } from "./approval-bundle.js";
...
function buildFixtureContext({ expired = false, hashMismatch = false, signatureMismatch = false, executable = false, bundleHashMismatch = false, missingBundleHash = false, currentBundleChanged = false } = {}) {
  ...
  const currentBundle = currentBundleChanged
...
function evaluateFixtureContext(context) {
  ...
  checks.push({ name: "approval-bundle-hash-match", passed: Boolean(token?.approvalBundleHash && actualBundleHash && token.approvalBundleHash === actualBundleHash), expectedHash: token?.approvalBundleHash || null, actualHash: actualBundleHash });
