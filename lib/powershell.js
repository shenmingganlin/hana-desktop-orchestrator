...
export function parseJsonOutput(result, label = "PowerShell") {
  ...
  try {
    ...
    const tail = output.slice(-160).replace(/\s+/g, " ");
    throw new Error(`${label} returned invalid JSON (outputLength=${output.length}, startsWith=${JSON.stringify(output.slice(0, 80))}, endsWith=${JSON.stringify(tail)})`);
  }
}
