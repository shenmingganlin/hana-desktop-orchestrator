...
export function warmup() {
  ...
  // Warm up uia-helper.exe (pre-load .NET Framework + UIA DLL into OS cache).
  try {
    spawnSync(UIA_HELPER_EXE, ["uia-tree", "0", "1"], { timeout: 5000, windowsHide: true });
  } catch { /* warmup is best-effort */ }
 ...
 */
export function runUiaHelper(verb, args = [], { timeoutMs = 15000 } = {}) {
  const exeArgs = [verb, ...args];
  try {
