...
import { findRecentLiveApprovalBundleRecord } from "./approval-store.js";
import { APPROVAL_TOKEN_VERSION, findRecentLiveApprovalTokenRecord, readApprovalTokenStore } from "./approval-token-store.js";
import { APPROVAL_BUNDLE_VERSION, hashApprovalBundle } from "./approval-bundle.js";
...
export function runExecutionPreflight({ recordId } = {}) {
  ...
  addCheck(checks, "token-version-supported", token?.version === APPROVAL_TOKEN_VERSION, {
    expectedVersion: APPROVAL_TOKEN_VERSION,
    actualVersion: token?.version ?? null,
  });
  ...
  const approvalBundleRecord = findRecentLiveApprovalBundleRecord({ target });
  const currentBundle = approvalBundleRecord?.bundle || null;
  const actualBundleHash = currentBundle ? hashApprovalBundle(currentBundle) : null;
  const currentBundleHash = typeof currentBundle?.bundleHash === "string" ? currentBundle.bundleHash : null;
  const currentBundleHashPresent = Boolean(currentBundleHash);
  const currentBundleHashIntegrity = Boolean(currentBundleHash && actualBundleHash && currentBundleHash === actualBundleHash);
  const currentBundleVersionSupported = currentBundle?.version === APPROVAL_BUNDLE_VERSION;
  addCheck(checks, "approval-bundle-fields-present", Boolean(token?.approvalBundleHash), {
    approvalBundleHash: token?.approvalBundleHash || null,
  });
  addCheck(checks, "approval-bundle-live", Boolean(approvalBundleRecord), {
    recordId: approvalBundleRecord?.id || null,
    target: currentBundle?.target || null,
  });
  addCheck(checks, "approval-bundle-version-supported", currentBundleVersionSupported, {
    expectedVersion: APPROVAL_BUNDLE_VERSION,
    actualVersion: currentBundle?.version ?? null,
  });
  addCheck(checks, "approval-bundle-hash-present", currentBundleHashPresent, {
    bundleHash: currentBundleHash,
  });
  addCheck(checks, "approval-bundle-hash-integrity", currentBundleHashIntegrity, {
    expectedHash: actualBundleHash,
    storedHash: currentBundleHash,
  });
  addCheck(checks, "approval-bundle-hash-match", Boolean(token?.approvalBundleHash && currentBundleHashIntegrity && token.approvalBundleHash === actualBundleHash), {
    expectedHash: token?.approvalBundleHash || null,
...
function computePreflightStatus(checks, passed, reason) {
  ...
  const criticalFailures = ["token-type", "token-version-supported", "token-non-executable", "token-hash-match"];
  ...
  if (failedNames.includes("approval-bundle-fields-present") || failedNames.includes("approval-bundle-live") || failedNames.includes("approval-bundle-version-supported") || failedNames.includes("approval-bundle-hash-present") || failedNames.includes("approval-bundle-hash-integrity") || failedNames.includes("approval-bundle-hash-match")) {
    return { status: "waiting", statusLabel: "waiting", headline: "Preflight waiting for a fresh approval bundle: the token is missing or detached from the current bundle." };
  }
