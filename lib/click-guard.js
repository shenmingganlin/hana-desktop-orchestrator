...
function Get-WinInfo([IntPtr]$h) {
  ...
  $targetProcessId = 0
  [HanaClickGuard]::GetWindowThreadProcessId($h, [ref]$targetProcessId) | Out-Null
  $name = $null
  try { $name = (Get-Process -Id $targetProcessId -ErrorAction Stop).ProcessName } catch {}
...
function runProbe(scriptBody, timeoutMs = 12000) {
  ...
  try {
    ...
    const res = spawnSync(
      ...
      { encoding: "utf-8", timeout: timeoutMs, windowsHide: true, maxBuffer: 32 * 1024 * 1024 }
    );
...
// expected: required { handle?, processName?, processId? } describing the intended
//   target window. A missing target is rejected; when present, the hit window must match.
// Returns { allowed, reason, hitWindow, foregroundWindow, checks }.
export function evaluateClickSafety({ x, y, expected = null } = {}) {
  const hasExpectedTarget = Boolean(expected && (expected.handle || expected.processId || expected.processName));
  if (!hasExpectedTarget) {
    return {
      allowed: false,
      reason: "no-expected-target",
      hitWindow: null,
      foregroundWindow: null,
      checks: [{ name: "expected-target-present", passed: false }],
    };
  }

  const probe = probeClickPoint({ x, y });
  const checks = [
    { name: "expected-target-present", passed: true },
  ];
  if (!probe.ok) {
    return {
      allowed: false,
