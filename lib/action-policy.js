...
export const ACTION_POLICIES = Object.freeze([
  ...
  { key: "window.close", title: "关闭窗口", group: "窗口", tool: "manage-window", defaultLevel: "confirm", configurable: true, hardConfirmation: false, warningOnChange: true, warning: "关闭窗口可能丢失未保存内容。应用可能弹出保存提示，但仍建议保留确认。" },
  ...
  { key: "element.type", title: "UIA ValuePattern 输入", group: "UIA 控制", tool: "type-element", defaultLevel: "auto", configurable: true, warning: "输入内容会写入目标控件。密码、凭据等内容仍按高风险分类。" },
  { key: "input.keyboard-fallback", title: "键盘回退输入", group: "输入回退", tool: "type-element", defaultLevel: "confirm", configurable: true, warningOnChange: true, warning: "会向已聚焦窗口发送真实键盘事件。修改后可能向错误的前台窗口输入内容。" },
  { key: "input.clipboard-fallback", title: "剪贴板回退输入", group: "输入回退", tool: "type-element", defaultLevel: "confirm", configurable: true, warningOnChange: true, warning: "会写入系统剪贴板并粘贴到前台窗口。修改后可能向错误的前台窗口输入内容。" },
  ...
  { key: "mouse.protected-click", title: "受保护坐标点击", group: "鼠标控制", tool: "protected-click", defaultLevel: "confirm", configurable: true, warning: "这是无 UIA 语义的坐标输入，必须依赖窗口守卫。" },
...
export function loadDesktopOrchestratorConfig() {
  try {
    if (!fs.existsSync(CONFIG_PATH)) return {};
    let raw = fs.readFileSync(CONFIG_PATH, "utf8");
    if (raw.charCodeAt(0) === 0xFEFF) raw = raw.slice(1);
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed.global === "object" ? parsed.global : {};
  } catch {
    return {};
  }
}
