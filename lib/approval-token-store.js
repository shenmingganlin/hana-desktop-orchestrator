...
export const APPROVAL_TOKEN_VERSION = 2;
...
export function saveApprovalToken(token, { source = "widget", ttlMs } = {}) {
  ...
  if (token.version !== APPROVAL_TOKEN_VERSION) {
    return { ok: false, reason: "unsupported-approval-token-version", expectedVersion: APPROVAL_TOKEN_VERSION, actualVersion: token.version ?? null, storePath: STORE_PATH };
  }
  if (typeof token.approvalBundleHash !== "string" || token.approvalBundleHash.length === 0) {
    return { ok: false, reason: "approval-bundle-hash-required", storePath: STORE_PATH };
  }
