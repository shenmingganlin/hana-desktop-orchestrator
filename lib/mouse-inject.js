...
//
// Prefers helper.exe (desktop-helper) over PowerShell for performance.
// Falls back to PowerShell silently on failure.
...
export function mouseDrag({ fromX, fromY, toX, toY, button = "left", steps = 24 }) {
  try {
    const result = spawnSync(HELPER_EXE, ["mouse-drag", String(fromX), String(fromY), String(toX), String(toY), button], {
      encoding: "utf8", timeout: 20000, windowsHide: true,
    });
    if (result.status === 0 && result.stdout) {
      return { ok: true, raw: { stdout: result.stdout } };
    }
  } catch { /* fallback */ }
...
export function mouseWheel({ x, y, notches = -3, axis = "vertical" }) {
  try {
    const result = spawnSync(HELPER_EXE, ["mouse-wheel", String(x), String(y), String(notches), axis], {
