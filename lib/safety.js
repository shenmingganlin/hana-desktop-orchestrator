...
export function requireRealInputApproval(input = {}, config = {}) {
  ...
  const skipConfirmation = config.skipConfirmationPhrase === true;
  ...
  // In strict security mode we never honor the skip flag. This keeps the final
  // human-confirmation gate alive even if a persisted config tries to disable it.
  if (!confirmationSatisfied && (securityMode === "strict" || !skipConfirmation)) {
    return {
      allowed: false,
      dryRun: true,
